...
class SparseBOPDMDOperator(BOPDMDOperator):
    ...
    :param num_eig_updates: Number of eigenvalue updates, i.e. number of
        Levenberg-Marquardt steps, to take every time an optimal mode matrix
        is computed (default = 1).
    :type num_eig_updates: int
    ...
    def _variable_projection(self, H, t, init_alpha, Phi, dPhi, amp_limit):
        ...
        for itr in range(self._maxiter):
            ...
            if self._verbose and itr < 3:
                ...
                plt.subplot(1, 2, 1)
                plt.title("Objective")
                plt.plot(self._obj_history[itr], ".-", c="k", mec="b", mfc="b")
                plt.semilogy()
                plt.subplot(1, 2, 2)
                plt.title("Error")
                plt.plot(self._err_history[itr], ".-", c="k", mec="r", mfc="r")
                plt.semilogy()
                plt.tight_layout()
                plt.show()
